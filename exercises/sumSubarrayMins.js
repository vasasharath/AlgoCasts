...
var sumSubarrayMins = function(A) {
  ...
  const stackLeft = [-1]
  for (let i = 0; i < N; i++) {
    while (stackLeft.length > 1 && A[i] < A[stackLeft[stackLeft.length - 1]]) stackLeft.pop()
  ...
  const stackRight = [N]
  for (let i = N - 1; i >= 0; i--) {
    while (stackRight.length > 1 && A[i] <= A[stackRight[stackRight.length - 1]]) stackRight.pop()
